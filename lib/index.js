...
/** Leading sample scanned for NUL bytes to reject binary reads (bytes). */
...
class SidebarGateway extends Service {
	...
	async readText(path, maxBytes, signal) {
		...
		// Read through the capped readBytes seam so the byte bound is enforced
		// before any buffering: a file that grows after lstat, or a symlink
		// whose target outgrew its link, fails with FS_TOO_LARGE instead of
		// buffering unbounded content in the host process. The bound is the
		// full read cap, not the caller's tail cap, so sub-cap previews still
		// truncate instead of failing.
		let capped;
		try {
			capped = await fs.readBytes(target, signal, MAX_READ_BYTES);
		} catch (error) {
			if (error instanceof Error && error.code === "FS_TOO_LARGE") {
				return { content: "", truncated: true, size: info !== void 0 && info.type === "file" ? info.size : undefined, tooLarge: true };
			}
			throw error;
		}
		const buffer = Buffer.from(capped);
		// readBytes skips the binary rejection readWholeText performed: keep it.
		if (buffer.subarray(0, BINARY_SAMPLE_BYTES).includes(0)) throw new SidebarError(`cannot read "${path}": binary file`);
		const limited = tail(buffer, cap);
	 ...
	 * @returns porcelain status rows, combined unstaged+cached diff, per-side
	 *   parts (unstaged/staged) for labeled rendering, truncation, and a repo
	 *   row (or null when the workspace is not a git repo or the identity
	 *   probes fail — repo info never breaks the diff itself).
	 */
	async diff(workspacePath, signal) {
		...
		if (statusRun.exitCode !== 0) {
			return { isGit: false, status: [], diff: "", parts: { unstaged: { text: "", truncated: false }, staged: { text: "", truncated: false } }, truncated: false, repo: null };
		}
		...
		return {
			...
			diff: limited.text,
			parts: {
				unstaged: { text: unstaged.stdout.text, truncated: unstaged.stdout.truncated },
				staged: { text: cached.stdout.text, truncated: cached.stdout.truncated }
			},
			truncated: limited.truncated || unstaged.stdout.truncated || cached.stdout.truncated,
			repo
		};
	}
	...
	async termSpawn(workspacePath, signal) {
		...
		// Serialize concurrent spawns for the same workspace: without this
		// check-then-act guard two overlapping termSpawn RPCs both pass the
		// reuse loop and both spawn, orphaning one PTY (alive, holding its
		// ring and listener, killable only by guessing its id). Later callers
		// await the in-flight spawn and land on its result.
		const pending = this.pendingSpawns.get(cwd);
		...
		const spawn = (async () => {
			...
			terminal.workspacePath = cwd;
			this.terminals.set(id, terminal);
			return { id, motd: `dsh-sidebar: terminal ${id} (pid ${handle.pid})` };
		})();
		this.pendingSpawns.set(cwd, spawn);
		try {
			return await spawn;
		} finally {
			// clear only if this spawn still owns the slot (a subsequent caller
			// may have awaited the same promise)
			if (this.pendingSpawns.get(cwd) === spawn) this.pendingSpawns.delete(cwd);
		}
	}
	 ...
	 * @param termId - terminal id from termSpawn.
	 * @param text - exact bytes to write; must be a string (missing or
	 *   non-string args are rejected instead of coerced into shell input).
	...
	async termWrite(termId, text, signal) {
		const terminal = this.terminals.get(termId);
		if (terminal === void 0) throw new SidebarError(`dsh-sidebar: unknown terminal "${termId}"`);
		if (typeof text !== "string") throw new SidebarError(`dsh-sidebar: termWrite text must be a string, got ${text === void 0 ? "no argument" : typeof text}`);
		if (terminal.exited) return { ok: false, exited: true };
		await terminal.handle.write(text);
		return { ok: true };
	}
