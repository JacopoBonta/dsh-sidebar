...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Compact display path for the agent row: relative to the diffed repo
     * root when the directory sits inside it (`.worktrees/agent-cwd`),
     * the tail of the absolute path when it belongs to an unrelated repo
     * (the row's title keeps the full path). Returns null when no
     * compaction applies — the caller falls back to the absolute path.
     * Shared by the repo bar and the tests.
     */
    function agentDisplayPath(repo, agentCwd) {
      if (typeof agentCwd !== "string" || agentCwd === "") return null;
      const target = agentCwd.replace(/\/+$/, "");
      if (target === "") return null;
      const root = repo && typeof repo.root === "string" ? repo.root.replace(/\/+$/, "") : "";
      ...
      return h("div", { className: "dsh-sb-foot" },
        ...
        repo
          ? h("div", { className: "dsh-sb-repobar" },
              ...
              repo.isWorktree && !seg ? h("span", { className: "dsh-sb-repochip", title: "main worktree: " + (repo.mainRoot || "?") }, "worktree") : null,
