...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function resolveShellCwd(workdir, sessionCwd) {
      const session = typeof sessionCwd === "string" && sessionCwd !== "" ? sessionCwd.replace(/\/+$/, "") : "";
      ...
      // relative: joins under the session workspace when one is known
      if (session) return normalizeSegments(session + "/" + workdir);
     ...
     * subdirectory still lands. Rows come from the diff payload's
     * `repo.worktrees` (`worktree list --porcelain` paths).
    ...
    function matchWorktree(worktrees, cwd) {
      ...
      for (const row of worktrees) {
        ...
        if (root !== "" && target.startsWith(root + "/")) return row;
      }
      return null;
    }
    ...
    const css = [
      ...
      ".dsh-sb-repobar{flex:none;display:flex;align-items:center;gap:8px;padding:7px 12px;border-top:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);font-size:12px;min-width:0}",
     ...
     * `workdir`. Returns null for other tools, malformed arguments, and an
     * omitted workdir — the scanner resolves an omitted one against the
     * session cwd through the call resolution path instead.
     * @param event - a `tool/call` SessionEvent.
     * @returns the raw workdir string, or null.
     */
    ...
    function useAgentActiveDir() {
      ...
      useEffect(() => {
          ...
          if (id !== watched) {
            ...
            if (id) {
              ...
              if (next && typeof next.getSnapshot === "function" && typeof next.subscribe === "function") {
                source = next;
                scan(sessionCwd);
                unsubscribeEvents = source.subscribe(() => scan(sessionCwd));
              }
            }
        ...
        adopt();
        return sessionsFace.list.subscribe(adopt);
      }, [sessionsFace]);
      return observation;
    }
    ...
    function DiffTab(props) {
      ...
      const onSwitchWorkspace = props.onSwitchWorkspace;
      ...
      // agent active-directory row: the directory the current session's agent
      // is working in (its latest observed shell call), with the worktree
      // branch when that directory belongs to the diffed repo, and a
      // one-click switch so the panel diffs that worktree instead
      const isHere = agentCwd !== null && samePath(agentCwd, wsPath || "");
      const agentWorktree = agentCwd && data && data.repo ? matchWorktree(data.repo.worktrees, agentCwd) : null;
      const agentRow = agentCwd
        ? h("div", { className: "dsh-sb-agentrow" },
            h("span", { className: "dsh-sb-agentdot" + (isHere ? " dsh-sb-agentdot-here" : " dsh-sb-agentdot-away"), "aria-hidden": "true" }),
            h("span", { className: "dsh-sb-agentlabel" }, isHere ? "agent working here" : "agent active"),
            h("span", { className: "dsh-sb-agentpath", title: agentCwd }, agentCwd),
            agentWorktree ? h("span", { className: "dsh-sb-repochip", title: "branch: " + (agentWorktree.branch || "detached") }, agentWorktree.branch || "detached") : null,
            !isHere && onSwitchWorkspace
              ? h("button", {
                  className: "dsh-sb-btn dsh-sb-btnic dsh-sb-agentswitch",
                  onClick: () => onSwitchWorkspace(agentCwd),
                  title: "Diff this directory instead — " + agentCwd,
                  "aria-label": "Diff the agent's active directory",
                }, "switch")
              : null)
        : null;
    ...
    function RepoInfoBar(props) {
      ...
      return h("div", { className: "dsh-sb-repobar" },
    ...
    function Panel() {
      ...
      const tree = h("div", {
        ...
        tab === "diff" ? h(DiffTab, { wsPath, agentCwd, onSwitchWorkspace: setWsPath }) : null,
        tab === "terminal" ? h(TermTab, { wsPath }) : null,
        h(RepoInfoBar, { wsPath }));
    ...
    exports.internals = { samePath, normalizeSegments, resolveShellCwd, matchWorktree, shellCallWorkdir, sessionRow };
