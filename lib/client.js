...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function RepoInfoBar(props) {
      ...
      // "agent active" away), the directory, and the worktree branch when
      // that directory belongs to the diffed repo. The panel follows it
      // automatically — no switch button.
      ...
      const agentRow = seg
        ? h("div", { className: "dsh-sb-agentrow" + (repo ? "" : " dsh-sb-agentrow-only") },
            ...
            seg.worktree ? h("span", { className: "dsh-sb-repochip", title: "branch: " + (seg.worktree.branch || "detached") }, seg.worktree.branch || "detached") : null)
        : null;
    ...
    function Panel() {
      ...
      // when that session has one.
      const registeredRef = useRef(null);
      useEffect(() => {
        const cwd = observation && observation.cwd;
        if (!cwd) { followedRef.current = null; return; }
        if (followedRef.current === cwd) return;
        followedRef.current = cwd;
        const registered = registeredRef.current;
        if (registered && registered.size > 0) {
          if (registered.has(cwd) || [...registered].some((p) => cwd.startsWith(p + "/"))) setWsPath(cwd);
          return;
        }
        ...
        rpc("workspaces", {}).then((res) => {
          ...
          if (set.has(cwd) || [...set].some((p) => cwd.startsWith(p + "/"))) setWsPath(cwd);
        }, () => {
          // RPC failed: follow anyway — host-side rejection surfaces in the
          // tab's existing error UI, never breaking the panel
          if (cancelled) return;
    ...
    exports.internals = { samePath, normalizeSegments, resolveShellCwd, matchWorktree, agentSegment, shellCallWorkdir, sessionRow };
